...
def main():
    ...
    parser.add_argument("--model", choices=["bilstm", "moe", "hardroute", "wta", "router", "transformer", "rl"], default="bilstm",
                        help="模型架构: bilstm | moe | hardroute | wta | router | transformer | rl")
    parser.add_argument("--contrastive", action="store_true",
                        help="使用对比预训练（bilstm / transformer / rl）")
        ...
        if args.model == "moe":
            ...
            if args.contrastive:
                ...
                contrastive_pretrain(X_train, feature_dim, pretrained_path,
                                      feature_cols=feature_cols,
                                      y_train=y_train)
                print("对比预训练完成，开始RL训练...")

            print(f"[3/5] {'全量' if args.mode == 'final' else ''}训练 {label}...")
            print("=" * 60)
            from src.models.train import train_final_rl, train_model_rl
            if args.mode == "final":
                model, val_metrics = train_final_rl(X_train, y_train, feature_dim,
                                                     pretrained_path=pretrained_path)
            else:
                model, val_metrics = train_model_rl(X_train, y_train, feature_dim,
                                                     pretrained_path=pretrained_path)
        else:
            label = "ConvBiLSTM"
            pretrained_path = None
    ...
    print("[4/5] 测试集推理...")
    print("=" * 60)
    from src.models.predict import predict, predict_transformer, predict_moe, predict_moe_wta, predict_hardroute, predict_router, predict_rl, predictions_to_dataframe

    if args.model == "rl":
        power, tc_arr, td_arr = predict_rl(X_test, feature_dim)
        predictions = None  # RL 不产价格预测
    elif args.model == "moe":
    ...
    if args.model == "rl":
        from src.strategy.optimizer import generate_strategy_from_decisions
        generate_strategy_from_decisions(power, test_times, OUTPUT_POWER_PATH)
    else:
