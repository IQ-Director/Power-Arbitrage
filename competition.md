...
$$
\text{Profit} = \sum_{t=0}^{95} P_t \cdot E_t
$$
电价序列通常呈现出**非平稳性、高波动性和多因素耦合特征**，其变化受到负荷变化、新能源出力、气象条件及市场行为等多种因素共同影响，因此预测难度较高。常见建模技术包括深度时间序列模型（如 LSTM、Transformer）、时序卷积网络（TCN）、信号分解方法（如 VMD、EMD）以及大模型方法等。参赛选手可根据自身思路选择适合的技术路线进行建模。
...
包括蒙西地区某节点（列名 A）的历史实时市场出清电价。

数据分辨率为 15 分钟，时间为北京时间。
...
- 充电开始时间：$0 \leq t_c \leq 80$
- 放电开始时间：$t_d \geq t_c + 8$ 且 $t_d \leq 88$
...
$$
\text{Score} = \frac{1}{D} \sum_{d=1}^{D} \text{Profit}_d
$$
收益越高，排名越靠前。
...
提交未来 D 日储能充放电策略的单个 CSV 文件。文件包含 96 × D 行（每 15 分钟一个预测点），索引为北京时间时间戳，列名为 `times/实时价格/power`。文件名为 `output.csv`。

参考格式为 `output_demo.csv` 
